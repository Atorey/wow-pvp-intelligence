...
- **Los nodos de talento tienen sitio reservado y ninguna forma que romper.** Cada nodo es una variable más de la misma lista, con la misma fila; no hay que rediseñar la caja para meterlos, que era la otra ventaja de no tener barras por categoría. Desde el [ADR 0026](../decisions/0026-talentos-por-nodo.md) el dato existe y está agregado por segmento, así que lo que queda es de UI, no de datos. Ojo a los denominadores: el n de esa lista es `talent_node_sample`, que no es el `gear_sample` ni el `talent_sample`.
