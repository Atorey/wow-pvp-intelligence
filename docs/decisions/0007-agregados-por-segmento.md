...
5. ~~**La actividad se mide por `captured_at`**~~ —"lo hemos vuelto a ver en el ladder dentro de la ventana"— mientras #16 no exista, y el filtro está aislado en una única función (`withinWindow`). **Sustituido el 19 de agosto de 2026 por el [ADR 0008](0008-ventana-de-actividad-por-partidas-jugadas.md)**: la actividad se deriva de la variación del contador de partidas.
...
- ~~**Cuando entre #16**~~, la ventana deja de medir "visto en el ladder" y pasa a medir "ha jugado". **Ocurrió el 19 de agosto de 2026** ([ADR 0008](0008-ventana-de-actividad-por-partidas-jugadas.md)): los agregados escritos antes de esa fecha están filtrados por "visto en el ladder" y no se recalculan hacia atrás, así que cualquier tendencia (#27) que cruce ese corte compara dos definiciones de población. Lo que sí distingue las filas nuevas es el reparto de evidencia (`active_by_delta` / `active_by_first_seen`), que `activity_window_days` no daba por sí sola.
- **El punto 8 ya tiene condición de revisión, y `excluded_search` ya está medido.** El 22 de agosto de 2026 valía **0 en las seis corridas existentes** —en toda la base hay 3 snapshots de búsqueda, de 2 personajes, de la temporada anterior—, así que la exclusión no ha costado nada todavía porque el tráfico que la haría cara no existe antes de lanzar. El [ADR 0011](0011-fuera-de-cobertura-se-describe-no-se-compara.md) mantiene la exclusión y fija cuándo deja de ser gratis: un par `(bracket, segmento)` con `excluded_search >= MIN_SAMPLE_MEDIUM` y `gear_sample < MIN_SAMPLE_MEDIUM`. Ahí anota también algo que no se tuvo en cuenta al escribir el punto 8: los snapshots de búsqueda **traen gear**, así que lo excluido no es solo población.
