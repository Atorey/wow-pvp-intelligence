...
- **Queda abierto el ruido de la ingesta**: el leaderboard se reingiere aunque su contenido no cambie en rating ni en partidas (el `content_hash` se mueve porque el `rank` baila con las altas y bajas), lo que añadió ~450.000 snapshots idénticos en un solo día. No afecta a la actividad —un contador que no sube no es actividad, se ingiera una vez o veinte—, pero sí al volumen y al coste, y se trata aparte en #53, junto con la política de retención (#48).
